from ririse02.chat.neo4j import Neo4jDriverManager as manager

cache_manager = manager.get_neo4j_cache_manager()
# global変数の設定（1日に1回（キャッシュのTTLが過ぎるたびに）読み込む（# 初回のデータ取得））
# NODE_LABELS = cache_manager. get_node_labels()
# RELATION_TYPES = cache_manager.get_relationship_types()
...
Output json format to single neo4j node without id.
{{Nodes: [{{"label":document type like manual, tutorial, article, news, report, journal, blog, code documentation, etc,
 "name":document name, "properties":dict}}],
"""
